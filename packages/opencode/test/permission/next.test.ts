...
// Permission precedence follows specificity. `evaluate()` picks the matching
// rule with the most literal (non-wildcard) characters in permission + pattern.
// Ties are broken by declaration order (last wins). This means a specific rule
// like "rg *" always beats a wildcard "*" regardless of config order.

test("fromConfig - specificity wins over declaration order", () => {
...
test("evaluate - more specific pattern wins regardless of order", () => {
  const result = Permission.evaluate("edit", "src/components/Button.tsx", [
    { permission: "edit", pattern: "src/components/*", action: "allow" },
    { permission: "edit", pattern: "src/*", action: "deny" },
  ])
...
test("evaluate - exact match wins over later wildcard", () => {
  const result = Permission.evaluate("bash", "/bin/rm", [
    { permission: "bash", pattern: "/bin/rm", action: "deny" },
    { permission: "bash", pattern: "*", action: "allow" },
  ])
